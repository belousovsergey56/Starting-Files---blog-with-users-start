...
def get_all_posts():
    ...
    admin = True if current_user.get_id() == str(1) else False
...
def login():
    ...
    if form.validate_on_submit():
        ...
        if not user:
            flash(message="Wrong email. Please try again")
...
def show_post(post_id):
    ...
    if form.validate_on_submit():
        ...
        new_comment = Comment(
            author_id=current_user.get_id(), post_id=post_id, text=form.comment.data
        )
